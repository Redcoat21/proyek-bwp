            ...
            @foreach($user as $user)
                @if($user->role == "LEC")
                    <div class="bg-white rounded shadow-md mx-2 my-5 px-5 py-5 flex items-center">
                        <img src="{{$user->profile_picture}}" alt="Image Lecturer" class="rounded-full w-28">
                        <div class="flex-col mx-5">
                            <div class="text-2xl font-bold text-black mb-1">{{$user->name}}</div>
                            <div class="text-xl font-italic text-gray-800">{{$user->description}}</div>
                        </div>
